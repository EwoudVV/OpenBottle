...
### a general save vault

the Screw Drivers recovery proved this is required. the launch transaction needs
per-game save discovery, an automatic local restore point, visible Steam Cloud
policy, retention controls, and hash verification. cloud access should be an
explicit game setting rather than an invisible side effect of Play.
